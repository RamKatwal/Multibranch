...
import { PurchaseReturnTable } from "@/components/reports/purchase-return-table"
...
} from "@/lib/reports/purchase-return"
import { cn } from "@/lib/utils"
import { purchaseReturnStatusLabels } from "@/types/purchase-return"
import type { PurchaseReturnStatus } from "@/types/purchase-return"
import type { ReportAsOfPreset } from "@/types/report"

const STATUS_OPTIONS = (
  Object.keys(purchaseReturnStatusLabels) as PurchaseReturnStatus[]
).map((value) => ({ value, label: purchaseReturnStatusLabels[value] }))

/** Purchase Return Register — the same booked returns as the Purchase Return report, presented as a tax register. */
export function PurchaseReturnRegisterReportPage() {
  ...
  const [statusFilter, setStatusFilter] = React.useState<
    PurchaseReturnStatus | "all"
  >("all")
  const [rowSize, setRowSize] = React.useState<DataTableRowSize>("md")
  const { isFullscreen, toggleFullscreen } = useDataTableFullscreen()

  const supplierOptions = React.useMemo(() => getPurchaseReturnSuppliers(), [])
  ...
  const rows = React.useMemo(
    ...
    [preset, customDate, supplierFilter, statusFilter]
  )
  ...
  const isAnyFilterActive =
    isAsOfActive || isSupplierFilterActive || isStatusFilterActive
  ...
  return (
    <div className="-mt-3 -mb-3 -ml-3 flex min-h-[calc(100svh-3.5rem)] md:-mt-4 md:-mb-4 md:-ml-4">
      <ReportListPanel
        activeHref="/reports/purchase-return-register"
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4 pt-3 pr-0 pb-3 pl-3 md:pt-4 md:pb-4 md:pl-4">
        ...
        >
          <div className="flex flex-col gap-3 border-b px-3 py-2.5 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <ReportStatusFilter
                ...
                value={statusFilter}
                onChange={(next) =>
                  setStatusFilter(next as PurchaseReturnStatus | "all")
                }
              />
              {isAnyFilterActive ? (
                <Button
          ...
          <PurchaseReturnTable rows={rows} rowSize={rowSize} />
        </div>
      </div>
    </div>
  )
