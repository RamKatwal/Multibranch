...
import { PurchaseRequisitionTable } from "@/components/reports/purchase-requisition-table"
import {
...
import { getPurchaseRequisitionReport } from "@/lib/reports/purchase-requisition"
...
const STATUS_OPTIONS = PURCHASE_REQUISITION_STATUSES.map((value) => ({
  value,
  label: purchaseRequisitionStatusLabels[value],
}))

export function PurchaseRequisitionReportPage() {
  ...
  const rows = React.useMemo(
    () =>
      getPurchaseRequisitionReport({ preset, customDate, statusFilter }),
  ...
  return (
    <div className="-mt-3 -mb-3 -ml-3 flex min-h-[calc(100svh-3.5rem)] md:-mt-4 md:-mb-4 md:-ml-4">
      <ReportListPanel
        activeHref="/reports/purchase-requisition"
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4 pt-3 pr-0 pb-3 pl-3 md:pt-4 md:pb-4 md:pl-4">
        ...
        >
          <div className="flex flex-col gap-3 border-b px-3 py-2.5 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <ReportStatusFilter
                ...
                value={statusFilter}
                onChange={(next) =>
                  setStatusFilter(next as PurchaseRequisitionStatus | "all")
                }
              />
              {isAnyFilterActive ? (
                <Button
          ...
          <PurchaseRequisitionTable rows={rows} rowSize={rowSize} />
        </div>
      </div>
    </div>
  )
