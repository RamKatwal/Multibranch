...
import type { DeliveryNoteReportRow } from "@/lib/reports/delivery-note"
...
export function DeliveryNoteTable({
  rows,
  rowSize,
  emptyMessage = "No delivery notes match the selected filters.",
      ...
      <table className={dataTableClassNames.table} data-row-size={rowSize}>
        <thead className="sticky top-0 z-10">
          <tr className={dataTableClassNames.headerRow}>
            ...
            <th className={headerCell}>Invoice Status</th>
                ...
                <tr key={row.id} className={dataTableClassNames.bodyRow}>
                  ...
                  <td className={cn(bodyCell, "font-mono text-xs")}>
                    <Link
                      ...
                      title="Open Delivery Note module"
                    >
                      {row.id}
                    </Link>
                  </td>
