...
import { InventoryMasterTable } from "@/components/reports/inventory-master-table"
...
import { getInventoryMasterReport } from "@/lib/reports/inventory-master"
import { cn } from "@/lib/utils"
import type { Branch } from "@/types/branch"

const CATEGORY_OPTIONS = productCategories.filter((category) => category !== "All")

export function InventoryMasterReportPage() {
  ...
  const rows = React.useMemo(() => {
    if (!branchId) return []
    return getInventoryMasterReport({
      branchId,
  ...
  return (
    <div className="-mt-3 -mb-3 -ml-3 flex min-h-[calc(100svh-3.5rem)] md:-mt-4 md:-mb-4 md:-ml-4">
      <ReportListPanel
        activeHref="/reports/inventory-master"
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4 pt-3 pr-0 pb-3 pl-3 md:pt-4 md:pb-4 md:pl-4">
        ...
        >
          ...
          <InventoryMasterTable rows={rows} rowSize={rowSize} />
        </div>
      </div>
    </div>
  )
