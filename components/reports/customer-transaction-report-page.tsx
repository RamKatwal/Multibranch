...
import { CustomerTransactionTable } from "@/components/reports/customer-transaction-table"
...
} from "@/lib/reports/customer-transaction"
import { cn } from "@/lib/utils"
import type { ReportAsOfPreset } from "@/types/report"

export function CustomerTransactionReportPage() {
  const [preset, setPreset] =
    React.useState<ReportAsOfPreset>(DEFAULT_AS_OF_PRESET)
  const [customDate, setCustomDate] = React.useState("")
  const [customerId, setCustomerId] = React.useState<string | "all">("all")
  const [rowSize, setRowSize] = React.useState<DataTableRowSize>("md")
  const { isFullscreen, toggleFullscreen } = useDataTableFullscreen()

  const customerOptions = React.useMemo(() => getCustomerOptions(), [])

  const rows = React.useMemo(
    () =>
      customerId === "all"
        ? []
        : getCustomerTransactionReport({ preset, customDate, customerId }),
    [preset, customDate, customerId]
  )

  return (
    <div className="-mt-3 -mb-3 -ml-3 flex min-h-[calc(100svh-3.5rem)] md:-mt-4 md:-mb-4 md:-ml-4">
      <ReportListPanel
        activeHref="/reports/customer-transaction"
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4 pt-3 pr-0 pb-3 pl-3 md:pt-4 md:pb-4 md:pl-4">
        ...
        >
          ...
          <CustomerTransactionTable rows={rows} rowSize={rowSize} />
        </div>
      </div>
    </div>
  )
