...
import { getFinancialOverviewReport } from "@/lib/reports/financial-overview"
import { cn } from "@/lib/utils"

/**
 * Cash/Bank position overview. No "As of" filter — the underlying accounts
 * (see lib/reports/financial-overview.ts) have no modeled movements yet, so
 * every balance is a fixed zero regardless of period.
 */
export function FinancialOverviewReportPage() {
  const [rowSize, setRowSize] = React.useState<DataTableRowSize>("md")
  const { isFullscreen, toggleFullscreen } = useDataTableFullscreen()

  const rows = React.useMemo(() => getFinancialOverviewReport(), [])

  return (
    <div className="-mt-3 -mb-3 -ml-3 flex min-h-[calc(100svh-3.5rem)] md:-mt-4 md:-mb-4 md:-ml-4">
      <ReportListPanel
        activeHref="/reports/financial-overview"
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4 pt-3 pr-0 pb-3 pl-3 md:pt-4 md:pb-4 md:pl-4">
        ...
        >
          ...
          <FinancialOverviewTable rows={rows} rowSize={rowSize} />
        </div>
      </div>
    </div>
  )
