...
import type { SalesByItemSummaryRow } from "@/lib/reports/sales-by-item-summary"
...
type SalesByItemSummaryTableProps = {
  rows: SalesByItemSummaryRow[]
  rowSize: DataTableRowSize
  emptyMessage?: string
}

export function SalesByItemSummaryTable({
  rows,
  rowSize,
  emptyMessage = "No sales match the selected filters.",
}: SalesByItemSummaryTableProps) {
            ...
            <>
              {rows.map((row) => (
                <tr key={row.productId} className={dataTableClassNames.bodyRow}>
                  ...
                  <td className={cn(bodyCell, "text-right tabular-nums")}>
                    {formatReportNumber(row.netSales)}
                  </td>
                  <td className={cn(bodyCell, "text-right tabular-nums")}>
