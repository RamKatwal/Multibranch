...
import { SalesRegisterTable } from "@/components/reports/sales-register-table"
...
import { getSalesBookCustomers } from "@/lib/reports/sales-book"
import { getSalesRegisterReport } from "@/lib/reports/sales-register"
import { cn } from "@/lib/utils"
import type { ReportAsOfPreset } from "@/types/report"

export function SalesRegisterReportPage() {
  ...
  const rows = React.useMemo(
    () => getSalesRegisterReport({ preset, customDate, customerFilter }),
  ...
  return (
    <div className="-mt-3 -mb-3 -ml-3 flex min-h-[calc(100svh-3.5rem)] md:-mt-4 md:-mb-4 md:-ml-4">
      <ReportListPanel
        activeHref="/reports/sales-register"
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4 pt-3 pr-0 pb-3 pl-3 md:pt-4 md:pb-4 md:pl-4">
        ...
        >
          ...
          <SalesRegisterTable rows={rows} rowSize={rowSize} />
        </div>
      </div>
    </div>
  )
