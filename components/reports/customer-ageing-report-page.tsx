...
import { CustomerAgeingTable } from "@/components/reports/customer-ageing-table"
...
import { getCustomerAgeingReport } from "@/lib/reports/customer-ageing"
import { cn } from "@/lib/utils"

export function CustomerAgeingReportPage() {
  const [rowSize, setRowSize] = React.useState<DataTableRowSize>("md")
  const { isFullscreen, toggleFullscreen } = useDataTableFullscreen()

  const rows = React.useMemo(() => getCustomerAgeingReport(), [])

  return (
    <div className="-mt-3 -mb-3 -ml-3 flex min-h-[calc(100svh-3.5rem)] md:-mt-4 md:-mb-4 md:-ml-4">
      <ReportListPanel
        activeHref="/reports/customer-ageing"
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4 pt-3 pr-0 pb-3 pl-3 md:pt-4 md:pb-4 md:pl-4">
        ...
        >
          ...
          <CustomerAgeingTable rows={rows} rowSize={rowSize} />
        </div>
      </div>
    </div>
  )
