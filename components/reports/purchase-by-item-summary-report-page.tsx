...
import { PurchaseByItemSummaryTable } from "@/components/reports/purchase-by-item-summary-table"
import {
...
import { getPurchaseByItemSummaryReport } from "@/lib/reports/purchase-by-item-summary"
import { cn } from "@/lib/utils"
import type { ReportAsOfPreset } from "@/types/report"

export function PurchaseByItemSummaryReportPage() {
  ...
  const productOptions = React.useMemo(() => getInventoryValuationProducts(""), [])

  const rows = React.useMemo(
    () => getPurchaseByItemSummaryReport({ preset, customDate, productIds }),
    [preset, customDate, productIds]
  )
  ...
  return (
    <div className="-mt-3 -mb-3 -ml-3 flex min-h-[calc(100svh-3.5rem)] md:-mt-4 md:-mb-4 md:-ml-4">
      <ReportListPanel
        activeHref="/reports/purchase-by-item-summary"
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4 pt-3 pr-0 pb-3 pl-3 md:pt-4 md:pb-4 md:pl-4">
        ...
        >
          ...
          <PurchaseByItemSummaryTable rows={rows} rowSize={rowSize} />
        </div>
      </div>
    </div>
  )
