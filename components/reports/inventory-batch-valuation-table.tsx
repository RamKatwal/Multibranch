...
import { formatReportNumber } from "@/lib/reports/inventory-batch-valuation"
...
type InventoryBatchValuationTableProps = {
  groups: BatchValuationGroup[]
...
export function InventoryBatchValuationTable({
  groups,
  rowSize,
  emptyMessage = "No products match the selected filters.",
}: InventoryBatchValuationTableProps) {
...
function InventoryBatchValuationRow({
  line,
  rowSize,
}: {
  line: BatchValuationLine
  rowSize: DataTableRowSize
}) {
  ...
  return (
    ...
    >
      ...
      <td className={cn(bodyCell, "text-muted-foreground tabular-nums")}>
        {line.expiresOn ? formatLongDate(line.expiresOn) : ""}
      </td>
