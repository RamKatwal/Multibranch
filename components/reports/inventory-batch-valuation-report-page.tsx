...
import { InventoryBatchValuationTable } from "@/components/reports/inventory-batch-valuation-table"
...
import {
  getBatchValuationProducts,
  getBatchValuationReport,
} from "@/lib/reports/inventory-batch-valuation"
import { cn } from "@/lib/utils"
import type { Branch } from "@/types/branch"
import type { ReportAsOfPreset } from "@/types/report"

export function InventoryBatchValuationReportPage() {
  ...
  const productOptions = React.useMemo(
    () => (branchId ? getBatchValuationProducts(branchId) : []),
    [branchId]
  )

  const groups = React.useMemo(() => {
    if (!branchId) return []
    return getBatchValuationReport({
      branchId,
      preset,
      customDate,
      productIds,
  ...
  return (
    <div className="-mt-3 -mb-3 -ml-3 flex min-h-[calc(100svh-3.5rem)] md:-mt-4 md:-mb-4 md:-ml-4">
      <ReportListPanel
        activeHref="/reports/inventory-batch-valuation"
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4 pt-3 pr-0 pb-3 pl-3 md:pt-4 md:pb-4 md:pl-4">
        ...
        >
          ...
          <InventoryBatchValuationTable groups={groups} rowSize={rowSize} />
        </div>
      </div>
    </div>
  )
