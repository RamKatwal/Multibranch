...
import { ReorderInventoryTable } from "@/components/reports/reorder-inventory-table"
...
import { getInventoryValuationProducts } from "@/lib/reports/inventory-valuation"
import { getReorderInventoryReport } from "@/lib/reports/reorder-inventory"
import { cn } from "@/lib/utils"
import type { Branch } from "@/types/branch"

const CATEGORY_OPTIONS = productCategories.filter((category) => category !== "All")

export function ReorderInventoryReportPage() {
  ...
  const rows = React.useMemo(() => {
    if (!branchId) return []
    return getReorderInventoryReport({
      branchId,
      categoryFilter: category,
      productIds,
    })
  }, [branchId, category, productIds])

  const isCategoryFilterActive = category !== "all"
  const isProductFilterActive = productIds !== "all"
  const isAnyFilterActive = isCategoryFilterActive || isProductFilterActive
  ...
  return (
    <div className="-mt-3 -mb-3 -ml-3 flex min-h-[calc(100svh-3.5rem)] md:-mt-4 md:-mb-4 md:-ml-4">
      <ReportListPanel
        activeHref="/reports/reorder-inventory"
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4 pt-3 pr-0 pb-3 pl-3 md:pt-4 md:pb-4 md:pl-4">
        ...
        >
          ...
          <ReorderInventoryTable rows={rows} rowSize={rowSize} />
        </div>
      </div>
    </div>
  )
