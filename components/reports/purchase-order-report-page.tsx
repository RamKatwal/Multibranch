...
import { PurchaseOrderTable } from "@/components/reports/purchase-order-table"
import {
...
} from "@/lib/reports/purchase-order"
...
const STATUS_OPTIONS = PURCHASE_ORDER_STATUSES.map((value) => ({
  value,
  label: purchaseOrderStatusLabels[value],
}))

export function PurchaseOrderReportPage() {
  ...
  const supplierOptions = React.useMemo(() => getPurchaseOrderSuppliers(), [])
  ...
  return (
    <div className="-mt-3 -mb-3 -ml-3 flex min-h-[calc(100svh-3.5rem)] md:-mt-4 md:-mb-4 md:-ml-4">
      <ReportListPanel
        activeHref="/reports/purchase-order"
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4 pt-3 pr-0 pb-3 pl-3 md:pt-4 md:pb-4 md:pl-4">
        ...
        >
          <div className="flex flex-col gap-3 border-b px-3 py-2.5 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <ReportStatusFilter
                ...
                value={statusFilter}
                onChange={(next) =>
                  setStatusFilter(next as PurchaseOrderStatus | "all")
                }
              />
              {isAnyFilterActive ? (
                <Button
          ...
          <PurchaseOrderTable rows={rows} rowSize={rowSize} />
        </div>
      </div>
    </div>
  )
