...
import { CustomerLedgerTable } from "@/components/reports/customer-ledger-table"
...
import {
  ...
  getCustomerOptions,
} from "@/lib/reports/customer-ledger"
...
export function CustomerLedgerReportPage() {
  ...
  const rows = React.useMemo(
    () => getCustomerLedgerReport({ preset, customDate, customerFilter }),
  ...
  return (
    <div className="-mt-3 -mb-3 -ml-3 flex min-h-[calc(100svh-3.5rem)] md:-mt-4 md:-mb-4 md:-ml-4">
      <ReportListPanel
        activeHref="/reports/customer-ledger"
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4 pt-3 pr-0 pb-3 pl-3 md:pt-4 md:pb-4 md:pl-4">
        ...
        >
          ...
          <CustomerLedgerTable rows={rows} rowSize={rowSize} />
        </div>
      </div>
    </div>
  )
