...
import { SalesOrderTable } from "@/components/reports/sales-order-table"
...
const STATUS_OPTIONS = SALES_ORDER_STATUSES.map((value) => ({
  value,
...
export function SalesOrderReportPage() {
  ...
  const customerOptions = React.useMemo(() => getSalesOrderCustomers(), [])

  const rows = React.useMemo(
    () =>
      getSalesOrderReport({ preset, customDate, customerFilter, statusFilter }),
  ...
  return (
    <div className="-mt-3 -mb-3 -ml-3 flex min-h-[calc(100svh-3.5rem)] md:-mt-4 md:-mb-4 md:-ml-4">
      <ReportListPanel
        activeHref="/reports/sales-order"
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4 pt-3 pr-0 pb-3 pl-3 md:pt-4 md:pb-4 md:pl-4">
        ...
        >
          <div className="flex flex-col gap-3 border-b px-3 py-2.5 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex flex-wrap items-center gap-2">
              ...
              <ReportStatusFilter
                ...
                value={statusFilter}
                onChange={(next) =>
                  setStatusFilter(next as SalesOrderStatus | "all")
                }
              />
              {isAnyFilterActive ? (
                <Button
          ...
          <SalesOrderTable rows={rows} rowSize={rowSize} />
        </div>
      </div>
    </div>
  )
