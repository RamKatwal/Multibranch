...
import { FISCAL_YEAR_OPTIONS } from "@/lib/reports/purchase-by-item-monthly"
import { getVatSummaryReport } from "@/lib/reports/vat-summary"
...
export function VatSummaryReportPage() {
  ...
  const rows = React.useMemo(
    () => (fiscalYear === FISCAL_YEAR_OPTIONS[0] ? getVatSummaryReport() : []),
    [fiscalYear]
  )
  ...
  return (
    <div className="-mt-3 -mb-3 -ml-3 flex min-h-[calc(100svh-3.5rem)] md:-mt-4 md:-mb-4 md:-ml-4">
      <ReportListPanel
        activeHref="/reports/vat-summary"
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4 pt-3 pr-0 pb-3 pl-3 md:pt-4 md:pb-4 md:pl-4">
        ...
        >
          ...
          <VatSummaryTable rows={rows} rowSize={rowSize} />
        </div>
      </div>
    </div>
  )
