...
import { Annex5Table } from "@/components/reports/annex-5-table"
...
import { getAnnex5Report } from "@/lib/reports/annex-5"
import { cn } from "@/lib/utils"
import type { ReportAsOfPreset } from "@/types/report"

export function Annex5ReportPage() {
  ...
  const rows = React.useMemo(
    () => getAnnex5Report({ preset, customDate }),
    [preset, customDate]
  )
    ...
    <div className="-mt-3 -mb-3 -ml-3 flex min-h-[calc(100svh-3.5rem)] md:-mt-4 md:-mb-4 md:-ml-4">
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4 pt-3 pr-0 pb-3 pl-3 md:pt-4 md:pb-4 md:pl-4">
        ...
        >
          ...
          <Annex5Table rows={rows} rowSize={rowSize} />
        </div>
      </div>
    </div>
  )
