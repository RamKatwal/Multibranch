...
import { SupplierTransactionTable } from "@/components/reports/supplier-transaction-table"
...
import {
  getSupplierOptions,
  getSupplierTransactionReport,
} from "@/lib/reports/supplier-transaction"
import { cn } from "@/lib/utils"
import type { ReportAsOfPreset } from "@/types/report"

export function SupplierTransactionReportPage() {
  const [preset, setPreset] =
    React.useState<ReportAsOfPreset>(DEFAULT_AS_OF_PRESET)
  const [customDate, setCustomDate] = React.useState("")
  const [supplierId, setSupplierId] = React.useState<string | "all">("all")
  ...
  const rows = React.useMemo(
    () =>
      supplierId === "all"
        ? []
        : getSupplierTransactionReport({ preset, customDate, supplierId }),
    [preset, customDate, supplierId]
  )

  return (
    <div className="-mt-3 -mb-3 -ml-3 flex min-h-[calc(100svh-3.5rem)] md:-mt-4 md:-mb-4 md:-ml-4">
      <ReportListPanel
        activeHref="/reports/supplier-transaction"
      ...
      <div className="flex min-w-0 flex-1 flex-col gap-4 pt-3 pr-0 pb-3 pl-3 md:pt-4 md:pb-4 md:pl-4">
        ...
        >
          ...
          <SupplierTransactionTable rows={rows} rowSize={rowSize} />
        </div>
      </div>
    </div>
  )
