...
import type { DeliveryNoteReturnReportRow } from "@/lib/reports/delivery-note-return"

const COLUMN_COUNT = 7

const COLUMNS = ["Date", "Note ID", "Customer"] as const

type DeliveryNoteReturnTableProps = {
  rows: DeliveryNoteReturnReportRow[]
  rowSize: DataTableRowSize
  emptyMessage?: string
}

export function DeliveryNoteReturnTable({
  rows,
  rowSize,
  emptyMessage = "No returns match the selected filters.",
}: DeliveryNoteReturnTableProps) {
      ...
      <table className={dataTableClassNames.table} data-row-size={rowSize}>
        <thead className="sticky top-0 z-10">
          <tr className={dataTableClassNames.headerRow}>
            ...
            <th className={headerCell}>Delivery Note</th>
                ...
                <tr key={row.id} className={dataTableClassNames.bodyRow}>
                  ...
                  <td className={cn(bodyCell, "font-mono text-xs")}>
                    <Link
                      ...
                      title="Open Returns Delivery Note module"
                    >
                      {row.id}
                    </Link>
                  </td>
