...
import {
  ...
  upsertStockTransfer,
} from "@/lib/stock-transfer-2/storage"
...
export function EditStockTransferPage({ transferId }: { transferId: string }) {
  ...
  function handleSave(next: StockTransfer) {
    ...
    router.push(`/inventory/stock-transfer-2/${encodeURIComponent(next.id)}`)
  }
  ...
  const detailHref = `/inventory/stock-transfer-2/${encodeURIComponent(transfer.id)}`
